...
core code to search for bot name summoning
...
user_agent = cfg['credentials']['useragent']
r = praw.Reddit(user_agent = user_agent)
# TODO: this should be oauth based
r.login(cfg['credentials']['botname'],
        cfg['credentials']['botpassword'])

# edit this to change subs being considered reactionary
#subs that are checked when called
# TODO: these should be set by config file
...
with open(cfg['data']['messages_seen'], "r") as f:
...
for msg in r.get_mentions(limit=None):
    ...
    if msg.id not in posts_replied_to:
        bot_uname = '/'.join(['', 'u',
                              cfg['credentials']['botname']])
        username = msg.body[msg.body.find(bot_uname) + 21:]
        ...
        with open(cfg['data']['messages_seen'], "w") as f:
